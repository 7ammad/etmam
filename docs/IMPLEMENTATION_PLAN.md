...
> **Session Reference**: This plan was created after a comprehensive review of the dashboard design, content, and workflow connectivity based on the original Arabic requirements document.
...
│   ├── Confirmation dialog before push (user confirmed)
...
### Sync Logic
1. Receive scored tenders array
2. For each scored tender:
...
3. Return counts and any errors
...
### Manual Testing
1. **CRM Push**: Navigate to tender detail → Click Push to CRM → Verify dialog → Confirm → Check status updates
2. **Evaluation Sync**: Run `pnpm sync:evaluations` → Check evaluations appear in DB
3. **Upload**: Go to empty dashboard → Drop CSV file → Verify tenders appear
...
| Purpose | File Path |
|---------|-----------|
| CRM Push Action | `actions/crm.ts` |
| Tender Detail Page | `app/[locale]/dashboard/[tenderId]/page.tsx` |
| Pattern Reference (CRM) | `components/settings/crm-odoo-card.tsx` |
| Pattern Reference (Export) | `components/dashboard/export-odoo-card.tsx` |
| Import Action | `actions/tender.ts` |
| Sync API Pattern | `app/api/cron/sync/route.ts` |
| E2E Tests | `tests/e2e/crm-push.spec.ts` |
...
```
Read the implementation plan at docs/IMPLEMENTATION_PLAN.md and execute phases 1-4 in order, verifying each phase before moving to the next.
```
