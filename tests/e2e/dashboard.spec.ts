...
test.describe('Dashboard', () => {
  ...
  test('should show file upload area when button clicked', async ({ page }) => {
    ...
    const uploadButton = page.getByTestId('upload-tender-button')
  ...
  test('should navigate to English dashboard', async ({ page }) => {
    ...
    const h1 = page.getByRole('heading', { level: 1 })
    await expect(h1).toContainText(/Dashboard|ETMAM Platform/)
  })
  ...
  test('should have upload and add buttons or tender list', async ({ page }) => {
    ...
    const hasUpload = await page.getByTestId('upload-tender-button').isVisible().catch(() => false)
